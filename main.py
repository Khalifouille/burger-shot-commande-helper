...
sheet = client.open_by_key(fichier_id).get_worksheet(1)

votre_nom = "Juan Castillo"

def trouver_ligne(nom):
    try:
...
def ajouter_valeurs(ligne, valeurs):
    try:
        mises_a_jour = []
...
def confirmer_vente():
    try:
        ...
        ligne = trouver_ligne(votre_nom)
        if ligne:
            ajouter_valeurs(ligne, valeurs)
